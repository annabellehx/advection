...
#pragma omp parallel for num_threads(NTHREADS) default(none) shared(local_rows, local_cols, matrix)
...
#pragma omp parallel num_threads(NTHREADS) default(none) shared(matrix, local_rows, local_cols, send_up, send_down, send_left, send_right)
    {
...
#pragma omp parallel num_threads(NTHREADS) default(none) shared(matrix, local_rows, local_cols, recv_up, recv_down, recv_left, recv_right)
    {
        for (int i = 0; i < local_rows; ++i)
        {
...
{
#pragma omp parallel for num_threads(NTHREADS) default(none) shared(local_rows, matrix)
    for (int i = 0; i < local_rows + 2; ++i)
        free(matrix[i]);
...
#pragma omp parallel for num_threads(NTHREADS) default(none) shared(C_curr, local_rows, local_cols, L, dx, u, v, start_row, start_col)
    for (int i = 0; i < local_rows; ++i)
    {
    ...
    update_ghost_cells(C_curr, rank, size, local_rows, local_cols, NTHREADS, send_up, send_down,
    ...
    for (int n = 0; n < NT; ++n)
    {
#pragma omp parallel for num_threads(NTHREADS) default(none) shared(C_curr, C_next, local_rows, local_cols, dx, dt, u, v)
        for (int i = 0; i < local_rows; ++i)
            for (int j = 0; j < local_cols; ++j)
                lax_method(C_curr, C_next, dt, dx, u[j], v[i], i + 1, j + 1);

        update_ghost_cells(C_next, rank, size, local_rows, local_cols, NTHREADS, send_up, send_down,
                           send_left, send_right, recv_up, recv_down, recv_left, recv_right);
