...
void update_ghost_cells(double **matrix, int N)
{
    for (int i = 0; i < N; ++i)
    {
...
int advection(int N, double L, double T)
{
    double dx = L / (N - 1);
    double dt = 0.000125;
    int NT = T / dt;
    ...
    double **C_curr = initialize_matrix(N);
    double **C_next = initialize_matrix(N);
    ...
    free_matrix(C_curr, N);
    free_matrix(C_next, N);
    free(u);
    free(v);
